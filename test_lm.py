...
if __name__ == '__main__':
    ...
    parser.add_argument('--bert_vocab_file', type=str, default=None, required=True)
    ...
    dataset = encode_dataset_lm(args.data_path, tokenizer)
    idx_to_vocab = load_idx_to_token(bert_vocab_file)
    max_len = 256
    n_ctx = min(max([len(sent[:max_len]) for sent in dataset])+2, n_ctx)
    dataset = [['[CLS]'] + sent[:max_len] + ['[SEP]'] for sent in dataset]
    dataset = [tokenizer.convert_tokens_to_ids(sent) for sent in dataset]
    random.shuffle(dataset)
    split_index = 100
    train_data, valid_data = dataset[split_index:], dataset[:split_index]    
    ...
    print('n_train={}, n_valid={}, n_batch_train={}, n_updates_total={}'.format(n_train, n_valid, n_batch_train, n_updates_total))
    ...
    saver.restore(sess, '/nas/ltj/GPT/debug_lm/models/chinese_gpt_test_save/model.ckpt')
